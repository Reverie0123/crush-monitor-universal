...
- **中英双语界面**：右上角「EN / 中」一键切换（设置里也可以），立即生效并会记住；第一次打开按浏览器语言决定。切到 English 后，模型写的判断依据、整体解读也会用英文；回复建议始终和原话用同一种语言。
...
各版本的更新内容见 [CHANGELOG.md](CHANGELOG.md)。当前版本 v2.3.0：全英文使用，英文界面下模型也用英文写分析。
