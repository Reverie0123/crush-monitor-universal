...
export function incrementalJobs(
  ...
  changed: boolean,
  /** Which messages may be analyzed line by line (analysis level and date range). */
  include: (m: Message) => boolean = () => true,
): AnalysisRequest[] {
  ...
  if (historical)
    for (const e of relevantEvents(messages, events, messages.slice(-20)).slice(
      ...
      if (
        historical.messages.some(
          (m) => m.id === e.id && m.sender === "other" && include(m),
        )
      )
        jobs.push({
          task: "other_messages",
...
export function overviewJob(
  ...
  return {
    ...
    revision,
    relation,
    // The overview reads the whole relationship history, leaving headroom for
